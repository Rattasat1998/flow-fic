...
import { ChevronLeft, ChevronRight, Layers, Inbox, List, Eye, Heart } from 'lucide-react';
...
export default function CategoryPageClient({ initialStories, categoryId, categoryLabel, currentPage, limit }: CategoryPageClientProps) {
    ...
    const handleSignOut = useCallback(async () => {
        try {
            ...
            console.error('[CategoryPageClient] Sign out failed:', error);
        }
    }, [router, signOut]);
                ...
                <section className={styles.sectionHeader} style={{ marginBottom: '40px' }}>
                    <div className={styles.sectionHeaderLeft}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                            <div style={{ backgroundColor: '#8b5cf6', padding: '8px', borderRadius: '12px', display: 'flex' }}>
                                <Layers size={24} color="white" />
                            </div>
                            <h1 className={styles.sectionHeadline} style={{ marginBottom: 0 }}>{categoryLabel}</h1>
                        </div>
                        <p className={styles.sectionSubhead}>รวมเรื่องทั้งหมดในหมวด{categoryLabel} เรียงตามความนิยม</p>
                    </div>
                </section>
                ...
                {initialStories.length === 0 ? (
                    <div className={styles.railStateCard}>
                        ...
                        <div>
                            ...
                            <p className={styles.railStateText}>ยังไม่มีเรื่องในหมวดนี้ในขณะนี้ กรุณากลับมาตรวจสอบอีกครั้งในภายหลัง</p>
                        </div>
                    </div>
                ) : (
                    <>
